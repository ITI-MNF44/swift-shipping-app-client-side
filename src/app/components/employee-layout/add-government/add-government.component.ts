...
export class AddGovernmentEmployeeComponent {
  constructor(
    private GovernmentService: GovernmentService,
    private router: Router
  ) {}
  governmentForm = new FormGroup({
    governmentName: new FormControl('', [
      Validators.required,
  ...
  get governmentName() {
    return this.governmentForm.controls.governmentName;
  }

  addGovernment() {
    if (this.governmentForm.status == 'VALID') {
      this.GovernmentService.addGovernment(
        String(this.governmentName.value)
      ).subscribe({
        next: (data) => {
          console.log(data);
          Swal.fire({
          ...
          this.router.navigate(['/employee/governments']);
        },
        error: (error) => {
          console.log(error);
        },
