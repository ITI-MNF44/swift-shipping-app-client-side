...
//---------------------------------------------------------
  ...
  orderForm = new FormGroup({
    ...
    phone: new FormControl(null, [Validators.required,Validators.pattern(/^[0](10|11|12|15)[0-9]{8}$/)]),
    email   : new FormControl(null, [Validators.required, Validators.pattern(/[\w]{4,}[a-zA-Z0-9]{0,}\@(gmail|yahoo|hotmail)\.com$/)]), 
    ...
    totalWeight: new FormControl(null,[Validators.required, Validators.pattern(/^[1-9]{1}[0-9]*/)]),
    note   : new FormControl(), 
    orderCost: new FormControl('', [Validators.required,Validators.pattern(/^[1-9]{1}[0-9]*/)]),
    shippingCost: new FormControl(),
    villageName: new FormControl('', [Validators.required]),
  });


  get customerName(){return this.orderForm.controls.customerName;}
  get phone() { return this.orderForm.controls.phone; }
  get email() { return this.orderForm.controls.email; }
  get government() { return this.orderForm.controls.government; }
  get region() { return this.orderForm.controls.region; }
  get isShippedToVillage() { return this.orderForm.controls.isShippedToVillage; }
  ...
  get totalWeight() { return this.orderForm.controls.totalWeight; }
  get note() { return this.orderForm.controls.note; }
  get orderCost() { return this.orderForm.controls.orderCost; }
  get shippingCost() { return this.orderForm.controls.shippingCost; }
  get villageName() { return this.orderForm.controls.villageName; }
    ...
    if(this.orderForm.status=="VALID"){
      let orderDTO: IOrderDTO = {
       ...
       this.OrderService.addOrder(orderDTO).subscribe({
        next: (data)=>{console.log("order added successfully", data)},
        error: (error)=>{console.log(error);}
       })
